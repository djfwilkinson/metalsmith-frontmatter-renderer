...
function replace ({ filename, files, settings, jsTransformer, metadata }) {
  ...
  return new Promise((resolve) => {
    matchedPaths.map(pathKey => {
      files[filename][settings.out][pathKey] = jsTransformer.render(files[filename][settings.key][pathKey], settings.options, metadata).body
    })
    resolve()
  })
}
...
module.exports = options => (files, metalsmith, done) => {
  ...
  Promise.all(
    matchedFiles.map(filename =>
      replace({ filename, files, settings, jsTransformer, metadata: metalsmith.metadata() })
    )
  )
    .then(() => done())
